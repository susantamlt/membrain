...
class ClientsController extends Controller
{
    ...
    public function index() {
        $clients=DB::select( DB::raw("SELECT clients.*,(SELECT COUNT(*) FROM protal_user WHERE protal_user.client_id = clients.id) AS PortalUser, (SELECT COUNT(*) FROM campaigns WHERE campaigns.client_id = clients.id) AS TotalCampaigns FROM  clients  ORDER BY clients.name ASC"));
    ...
    public function upload(Request $request) {
            ...
            foreach ($data->toArray() as $key => $value) {
                ...
                if($type_file=='phone'){
                    if(!empty($value) && isset($value['phone_number'])){
                        ...
                        if($phonelength == 10 && $cunCode == 'AU'){
                            $valid = 1;
                        } elseif ($phonelength >= 9 && $phonelength <= 11 && $cunCode == 'NZ') {
                            $valid = 1;
                        } elseif ($phonelength >= 10 && $phonelength <= 11 && $cunCode == 'UK') {
                            $valid = 1;
                        } elseif ($phonelength == 11 && $cunCode == 'CA') {
                            $valid = 1;
                        } elseif ($phonelength == 11 && $cunCode == 'US') {
                            $valid = 1;
                        } elseif ($phonelength == 10 && !in_array($cunCode, $countrycode) && $cunCode!='') {
                            $valid = 1;
                        } elseif($phonelength >= 9 && $phonelength <= 11 && $cunCode==''){
                            $valid = 1;
                        } else {
                            $valid = 0;
                        }
        ...
        if($st!=1){
            ...
            if(!$request->myfile->move($path, $imageFileName)){
                $result = array('status'=>'0','fname'=>$imageFileName,'tmp_client_id'=>$tmp_client_id,'ccode'=>$cunCode);
            } else {
                $result = array('status'=>'1','fname'=>$imageFileName,'tmp_client_id'=>$tmp_client_id,'ccode'=>$cunCode);
            }
        } else {
            $result = array('status'=>'2','fname'=>'','tmp_client_id'=>'','ccode'=>$cunCode);
        }
